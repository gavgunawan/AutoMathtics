...
function parse(argv) {
  ...
  return { command, week: options['--week'] ?? null, familyId: options['--family'] ?? null, dryRun: flags.has('--dry-run'), days: Number(days), positional };
}
...
if (!reading && !/^[a-f0-9]{64,}$/.test(env.SESSION_SECRET || '')) throw Error('SESSION_SECRET is required: it signs the buttons in the email, which the service checks.');
let origin = null; try { origin = new URL(env.APP_ORIGIN).origin; } catch { /* refused below */ }
if (!reading && (!origin || origin !== env.APP_ORIGIN || (!emulator && !origin.startsWith('https://')))) throw Error('Set APP_ORIGIN to the app’s https origin, no path or trailing slash: the buttons open it.');
