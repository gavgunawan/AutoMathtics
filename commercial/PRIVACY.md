...
| Login sessions (opaque cookie hash, uid, role, family, CSRF token, expiry) | Firestore `sessions/{hash}` | nobody reads them but the server | 30 minutes (parent), TTL; ended at sign-out, deletion, recovery | expiry |
