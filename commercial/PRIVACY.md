...
- **Children have no account.** A child has a nickname, a PIN (stored only as a salted, peppered
  ...
  No email, no phone, no photo, nothing shared between families. The one free text a child writes is a
  question to the "Explain to me" tutor (19 Sep 2026): kept 24 hours with its session, sent with the question
  to Anthropic to answer, never with a name; a parent switches the tutor off in Game & progress.
...
| The tutor's thread (19 Sep 2026): what a child typed or said to "Explain to me" on one question and what the tutor answered, up to six turns, and that the session used the tutor | inside the practice session, `families/{f}/learning/{c}/sessions/{id}` (an Olympia visit is a session too) | the server; Anthropic receives each turn with the question and the child's age and year level to write the reply — never a name, an address or a number | 24 hours, the session's TTL | expiry |
| Tutor usage counts: explanations and messages a child asked for on a day; the calls the whole service made in a month | `families/{f}/learning/{c}/tutor/{date}`, `tutor/usage-{YYYY-MM}` | the server; the operator | 3 days (a child's day) and 400 days (a month's total), TTL | expiry |
| Olympia (19 Sep 2026): the visit under way, each moon's medals, the newest thirty visits (date, moon, score, medal), and the family's Olympia pass | the child's learning document; `families/{f}.olympia` | the parent (Game & progress); the child; the operator | with the family | family deletion |
...
address: the address itself, because writing to it on 14 September is the whole point of the list; the moment the
...
- ~~The parent-facing disclosure and consent version (`consentVersion`) updated from `pilot-v1`.~~ Done 13 Sep 2026: the Terms of
  ...
  cannot be sent without the terms box, and creating a family requires and records `consentVersion: terms-2026-09-13.4`. The words
