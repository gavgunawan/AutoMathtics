...
| **4.1 Stripe adapter** | the Stage 3 gateway contract against a real provider: hosted checkout, provider idempotency keys, `Stripe-Signature` verification, price ids → plans, refund facts from the provider object, superseded sessions expired at the provider, provider state fetched where event order is not total | none to build; **Stripe account in test mode** (free, no fees) to exercise — owner creates it |
| **4.2 reconciliation in the sandbox** | `reconciliation_required`, stale/frozen/superseded intents, provider-charged-but-local-finalisation-failed, webhook retries and out-of-order delivery, dunning, cancellations, refunds, reconciliation reports — with `stripe listen` forwarding real test-mode events to the local server | none (Stripe test mode + emulator) |
...
| **4.4 real MFA + recovery** | real SMS second factor (Identity Platform) and the lost-phone / changed-number recovery ceremony that cannot be socially engineered through support | **SMS cost per message**; owner's go; recovery policy written before code |
...
| **4.7 operational and privacy readiness** | runbooks (payment reconciliation, deletion/export verification, rollback), monitoring, backup assumptions, privacy disclosures and the retention table against the staging infrastructure | none |
