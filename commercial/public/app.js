...
function leavingScreen(family, e, source = 'app') {
  transientView = true;
  const box = panel('CANCEL OR PAUSE', family.label, 'Before anything changes: tell us why you are thinking of leaving. There may be an easier answer than cancelling.');
  onBack = refresh;
  leavingFacts(box, e);
  const picks = new Map();
  ...
  box.append(note500);
  box.append(button('Continue', async () => {
...
function leavingOffersScreen(family, e, view) {
  ...
  const box = panel('CANCEL OR PAUSE', hold ? 'Let us fix it first' : 'What would you like to do?',
    hold ? 'Tell us what went wrong and we will look into it. Nothing is cancelled by doing that.'
      : view.offers.length ? 'One of these may suit you better than leaving. Or carry on below.' : 'Here is what you can do.');
  onBack = refresh;
  leavingFacts(box, e);
  for (const offer of view.offers) {
    if (offer.kind === 'email_monthly') box.append(button('Send the progress report monthly instead of weekly', leavingSend(view, { action: 'reduce_email', cadence: 'monthly', offerAccepted: 'email_monthly' }, () => 'The progress report now comes once a month, on the first Monday. Your subscription is unchanged.'), 'primary'));
    if (offer.kind === 'email_off') box.append(button('Turn the progress report off, keep the subscription', leavingSend(view, { action: 'reduce_email', cadence: 'off', offerAccepted: 'email_off' }, () => 'The progress report is off. Your subscription is unchanged.'), 'ghost'));
    if (offer.kind === 'pause') {
      ...
      for (const months of offer.months) box.append(button(`Pause for ${months} month${months === 1 ? '' : 's'}`, leavingSend(view, { action: 'pause', months, offerAccepted: 'pause' }, () => `Paused for ${months} month${months === 1 ? '' : 's'}. Nothing more will be charged until it is over, and you can start again any time.`), 'ghost'));
    }
    if (offer.kind === 'downgrade' || offer.kind === 'seats') {
      ...
      box.append(button(offer.kind === 'downgrade' ? `Switch to the smaller ${name} plan at renewal (${offer.seats} slots)` : `Fewer seats at renewal: ${name}, ${offer.seats} slots`,
    ...
    if (offer.kind === 'feedback') box.append(button('Tell us what went wrong', () => {
  ...
  const exits = el('div', null, 'actions');
  exits.append(button('Keep everything as it is', leavingSend(view, { action: 'keep' }, () => 'Nothing was changed. Thank you for telling us.'), 'ghost'));
  if (hold) exits.append(button('I still want to cancel', () => leavingOffersScreen(family, e, { ...view, hold: false, offers: [] }), 'text-button'));
  else if (['trial', 'active', 'grace', 'paused'].includes(e?.state)) exits.append(button('Cancel my subscription', () => leavingCancelScreen(family, e, view), 'text-button'));
  exits.append(button('Back', refresh, 'ghost'));
  box.append(exits);
}
...
function leavingCancelScreen(family, e, view) {
  ...
  const box = panel('CANCEL OR PAUSE', 'Cancel the subscription?', paused
    ...
    : `The grid stays open until ${when || 'the end of the period you have paid for'} and is not renewed after that. Your children keep their profiles, their progress and their coins. You can subscribe again whenever you like.`);
  onBack = refresh;
  box.append(button('Back', () => leavingOffersScreen(family, e, view), 'ghost'), // Back first: the second tap of a double-tap lands here, not on Cancel
...
async function parentScreen() {
  ...
  if (e.state) { // a subscription: what it is and when it turns (dates are the server's; the browser only shows them)
    ...
    if (e.pause && e.state !== 'paused') box.append(el('p', `Paused: the period you have paid for runs to ${e.accessUntil ? new Date(e.accessUntil).toLocaleDateString() : 'its end'}${e.pause.resumesAt ? `, then nothing is collected until ${new Date(e.pause.resumesAt).toLocaleDateString()}` : ''}.`, 'notice'));
    if (e.pause) {
      const resumeOp = crypto.randomUUID();
      box.append(button('Start my subscription again now', async () => { await api('/billing/resume', { operationId: resumeOp }); note('The pause is over. Your next invoice comes as usual.'); await refresh(); }, 'primary'));
    }
    if (['trial', 'active', 'grace'].includes(e.state)) {
      ...
      if (e.cancelAtPeriodEnd) box.append(button('Keep my subscription', async () => { await api('/billing/cancel', { undo: true, operationId: cancelOp }); await refresh(); }, 'text-button'));
      else if (!e.pause) box.append(button('Cancel or pause', () => leavingScreen(family, e), 'text-button')); // the leaving flow: a reason, an offer, and only then the action
    ...
    if (e.state === 'paused') box.append(button('Cancel or pause', () => leavingScreen(family, e), 'text-button'));
    if (!['active', 'grace', 'paused'].includes(e.state)) planButtons(box, billing);
  } else if (!active) {
...
async function emailScreen(token) {
  ...
  if (d.action === 'unsub' && d.cadence === 'weekly') {
    box.append(el('p', 'Once a month may be enough: one email on the first Monday, covering four weeks.', 'notice'),
      button('Send it monthly instead', apply({ cadence: 'monthly' }), 'primary'),
      button('No, stop the report', apply({ cadence: 'off' }), 'ghost'));
  } else {
    box.append(button('Confirm', apply({}), 'primary'));
  }
  box.append(button('Cancel', async () => { await refresh(); note('Nothing was changed.'); }, 'ghost'));
  if (d.action === 'unsub' && model?.role === 'parent' && model.family) { // signed in on this device: the whole flow is one tap away
    box.append(button('It is not just the email — cancel or pause', () => leavingScreen(model.family, model.family.entitlement, 'email'), 'text-button'));
  }
}
