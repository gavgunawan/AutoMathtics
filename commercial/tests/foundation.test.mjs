...
test('family setup is idempotent and starts with zero paid access', async () => {
  ...
  const again = await f.service.createFamily(a.ctx, { label: 'Test family', adultAttestation: true, consentVersion: 'terms-2026-09-13.4' });
...
test('handover barrier also covers an identity-provider clock ahead of the server', async () => {
  ...
  const created = await f.service.createFamily(a.ctx, { label: 'Test', adultAttestation: true, consentVersion: 'terms-2026-09-13.4' });
