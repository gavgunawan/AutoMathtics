...
test('what the second verification found: the same operation twice never expires the session the parent holds; a subscription paid at the provider while the record lags is never ended; the customer by its recorded id; a deleted customer settles the debt; a refund of the previous subscription leaves the new one alone; a held upgrade answers every attempt; the provider\'s answer survives a failed finalisation; the deletion expires the session; changes bind to the record\'s subscription; paused is ended; the refusal row carries its code', async () => {
  ...
  const h = rig(), c = await pastDue(h, 'parentC'); h.account.state.sub.status = 'active'; h.account.state.sub.cancel_at_period_end = true; const opH = randomUUID();
  assert.ok((await h.payments.checkout(c.ctx, { plan: 'family', operationId: opH })).url); assert.equal(h.account.state.sub.status, 'canceled');
  const seenH = (await h.f.store.get(`checkouts/stripe:${opH}`)).endPrevious.result; assert.equal(seenH.status, 'active'); assert.equal(seenH.cancelAtPeriodEnd, true); assert.equal(seenH.periodEnd, h.account.state.sub.current_period_end * 1000, 'what the subscription looked like right before the ending is on the record');
  ...
  assert.equal(applied.providerOperationRef, iq.providerOperationRef, 'the recorded answer is the one applied, not the retry\'s replay'); assert.equal(applied.proration.invoiceRef, iq.proration.invoiceRef); assert.equal(applied.result.proration.invoiceRef, iq.proration.invoiceRef);
...
test('a first checkout paid at the provider and clicked again before its completion landed: the subscription names the checkout, which is reinstated — never left superseded with its payment refused — and a completion the inbox rejected meanwhile is processed again; a subscription naming no checkout of ours is the operator\'s', async () => {
  ...
  await assert.rejects(h.payments.checkout(c.ctx, { plan: 'starter', operationId: randomUUID() }), rejected('USE_PLAN_CHANGE'), 'the requeued completion applied: a paid family, unmarked');
  assert.equal((await family(h.f, c.familyId)).subscription?.plan, 'starter', 'the rejected completion was processed again'); assert.equal((await family(h.f, c.familyId)).providerAttention, undefined); assert.equal((await h.f.store.get(`checkouts/stripe:${opD}`)).status, 'completed'); assert.equal((await h.f.store.get(`billingEvents/stripe:${late.id}`)).outcome.status, 'applied');
...
test('the deletion\'s expiries: a provider fault is recorded on the checkout and never fatal, and the sweep names the session left payable; a rerun after a crash before the record asks again, and a session no longer open is the settled state; a crash after the ending, before its record, resumes on the provider\'s own notice', async () => {
  ...
  await x.support.requestDeletion(y.ctx, op()); await x.support.executeDeletion(y.familyId, { operator: OPERATOR, force: true });
  ...
  assert.ok((await x.support.inspectAll({ operator: OPERATOR })).findings.some((fnd) => fnd.code === 'DELETED_FAMILY_PROVIDER_LIVE' && fnd.family === y.familyId && /could not be expired/.test(fnd.detail)), 'the sweep names the session left payable while the provider is down');
  x.account.gw.fetch = realFetch; assert.ok(!(await x.support.inspectAll({ operator: OPERATOR })).findings.some((fnd) => fnd.family === y.familyId), 'the provider back: the next sweep expires it and the finding clears');
  assert.deepEqual((await x.f.store.get(`checkouts/stripe:${opR2}`)).expiredByDeletion, { expired: true, at: x.f.now() }); assert.ok(x.account.state.expired.some((p) => p.includes(R2.providerCheckoutRef)));
...
test('the third verification\'s pins on the checkout seam: a failed debit releases the checkout; the clearing mark survives a second click\'s window and the deletion ends what it made; a paid checkout is reinstated whichever click superseded it; the ending reads the subscription once more before deleting', async () => {
  // a bank debit that fails: the checkout is released, the next click starts afresh
  const r = rig(), { f, account, payments } = r, a = await f.family('parentA', 0), opA = randomUUID(), A = await payments.checkout(a.ctx, { plan: 'starter', operationId: opA }); assert.ok(A.url);
  account.activate(PRICES.starter, undefined, { metadata: { checkoutId: opA } });
  const session = { object: 'checkout.session', customer: account.state.customer.id, subscription: 'sub_1', client_reference_id: opA, metadata: { familyId: a.familyId, checkoutId: opA } };
  let sg = signed(f, event(f, 'checkout.session.completed', { ...session, payment_status: 'unpaid' })); assert.equal((await payments.receive('stripe', sg.raw, sg.headers)).reason, 'PAYMENT_PENDING');
  await assert.rejects(payments.checkout(a.ctx, { plan: 'starter', operationId: randomUUID() }), rejected('CHECKOUT_COMPLETING'));
  sg = signed(f, event(f, 'checkout.session.async_payment_failed', { ...session, payment_status: 'unpaid' })); assert.deepEqual(await payments.receive('stripe', sg.raw, sg.headers), { status: 'ignored', reason: 'PAYMENT_FAILED' });
  const failed = await f.store.get(`checkouts/stripe:${opA}`); assert.equal(failed.paymentPending, null); assert.equal(failed.paymentFailed.subscriptionRef, 'sub_1'); assert.equal(failed.paymentFailed.was.subscriptionRef, 'sub_1');
  account.state.sub.status = 'canceled'; // the provider ends the subscription whose first payment failed
  const opB = randomUUID(); assert.ok((await payments.checkout(a.ctx, { plan: 'starter', operationId: opB })).url, 'the next click starts afresh'); assert.equal((await f.store.get(`checkouts/stripe:${opA}`)).status, 'superseded'); assert.equal(sessions(account), 2);
  // the unpaid completion landing inside a second click's window (the checkout transiently superseded): remembered all the same, the mark travels with the reinstatement, the third click is refused before any provider call, and a deletion ends the subscription
  const g = rig(), b = await g.f.family('parentB', 0), opB2 = randomUUID(), B2 = await g.payments.checkout(b.ctx, { plan: 'starter', operationId: opB2 }); assert.ok(B2.url);
  g.account.activate(PRICES.starter, undefined, { metadata: { checkoutId: opB2 } });
  const unpaid = event(g.f, 'checkout.session.completed', { object: 'checkout.session', payment_status: 'unpaid', customer: g.account.state.customer.id, subscription: 'sub_1', client_reference_id: opB2, metadata: { familyId: b.familyId, checkoutId: opB2 } });
  const realInspect = g.account.gw.inspect.bind(g.account.gw); g.account.gw.inspect = async (...args) => { g.account.gw.inspect = realInspect; const s2 = signed(g.f, unpaid); assert.equal((await g.payments.receive('stripe', s2.raw, s2.headers)).reason, 'PAYMENT_PENDING'); return realInspect(...args); };
  await assert.rejects(g.payments.checkout(b.ctx, { plan: 'starter', operationId: randomUUID() }), rejected('CHECKOUT_COMPLETING'));
  const back = await g.f.store.get(`checkouts/stripe:${opB2}`); assert.equal(back.status, 'pending'); assert.equal(back.paymentPending.subscriptionRef, 'sub_1', 'the mark travelled with the reinstatement');
  const calls = g.account.calls.length; await assert.rejects(g.payments.checkout(b.ctx, { plan: 'starter', operationId: randomUUID() }), rejected('CHECKOUT_COMPLETING')); assert.equal(g.account.calls.length, calls, 'refused from the first transaction: no provider call'); assert.equal((await g.f.store.get(`checkouts/stripe:${opB2}`)).status, 'pending');
  await g.support.requestDeletion(b.ctx, op()); const recB = await g.support.executeDeletion(b.familyId, { operator: OPERATOR, force: true });
  assert.equal(g.account.state.sub.status, 'canceled'); assert.equal((await g.f.store.get(`checkouts/stripe:${opB2}`)).endedByDeletion.cancelled, true); assert.equal(recB.providerCancellation.status, 'cancelled');
  assert.ok(!(await g.support.inspectAll({ operator: OPERATOR })).findings.some((x) => x.family === b.familyId), 'ended: the sweep is quiet');
  // a bank-debit checkout that completed normally, then the family deleted through its record: no finding either; one whose ending faults at deletion is named
  const h = rig(), c = await h.f.family('parentC', 0), opC = randomUUID(); assert.ok((await h.payments.checkout(c.ctx, { plan: 'starter', operationId: opC })).url); h.account.activate(PRICES.starter, undefined, { metadata: { checkoutId: opC } });
  const sessionC = { object: 'checkout.session', customer: h.account.state.customer.id, subscription: 'sub_1', client_reference_id: opC, metadata: { familyId: c.familyId, checkoutId: opC } };
  sg = signed(h.f, event(h.f, 'checkout.session.completed', { ...sessionC, payment_status: 'unpaid' })); assert.equal((await h.payments.receive('stripe', sg.raw, sg.headers)).reason, 'PAYMENT_PENDING');
  sg = signed(h.f, event(h.f, 'checkout.session.async_payment_succeeded', { ...sessionC, payment_status: 'paid' })); assert.equal((await h.payments.receive('stripe', sg.raw, sg.headers)).status, 'applied'); assert.match((await h.f.store.get(`checkouts/stripe:${opC}`)).paymentPending.completedBy, /^evt_/, 'the mark says which completion settled it');
  await h.support.requestDeletion(c.ctx, op()); const recC = await h.support.executeDeletion(c.familyId, { operator: OPERATOR, force: true }); assert.equal(recC.providerCancellation.status, 'cancelled'); assert.equal(h.account.state.sub.status, 'canceled');
  assert.ok(!(await h.support.inspectAll({ operator: OPERATOR })).findings.some((x) => x.family === c.familyId), 'completed through the record: nothing left');
  const k = rig(), d = await k.f.family('parentD', 0), opD = randomUUID(); assert.ok((await k.payments.checkout(d.ctx, { plan: 'starter', operationId: opD })).url); k.account.activate(PRICES.starter, undefined, { metadata: { checkoutId: opD } });
  sg = signed(k.f, event(k.f, 'checkout.session.completed', { object: 'checkout.session', payment_status: 'unpaid', customer: k.account.state.customer.id, subscription: 'sub_1', client_reference_id: opD, metadata: { familyId: d.familyId, checkoutId: opD } })); assert.equal((await k.payments.receive('stripe', sg.raw, sg.headers)).reason, 'PAYMENT_PENDING');
  const realFetchK = k.account.gw.fetch; k.account.gw.fetch = async (url, init) => { if ((init?.method || 'GET') === 'DELETE') throw Error('ECONNRESET'); return realFetchK(url, init); };
  await k.support.requestDeletion(d.ctx, op()); const recD = await k.support.executeDeletion(d.familyId, { operator: OPERATOR, force: true }); k.account.gw.fetch = realFetchK;
  assert.equal(recD.providerCancellation.status, 'failed'); assert.equal(recD.providerCancellation.reason, 'PROVIDER_UNREACHABLE'); assert.equal((await k.f.store.get(`checkouts/stripe:${opD}`)).endedByDeletion.cancelled, false);
  assert.ok((await k.support.inspectAll({ operator: OPERATOR })).findings.some((x) => x.code === 'DELETED_FAMILY_PROVIDER_LIVE' && x.family === d.familyId && /was not ended at deletion/.test(x.detail)), 'the sweep names the subscription left billing');
  // a paid first checkout is reinstated whichever click superseded it: after a click that faulted, a later click under a new operation reinstates it and its completion applies
  const m = rig(), e = await m.f.family('parentE', 0), opE = randomUUID(), E = await m.payments.checkout(e.ctx, { plan: 'starter', operationId: opE }); assert.ok(E.url);
  m.account.activate(PRICES.starter, undefined, { metadata: { checkoutId: opE } });
  const restore = down(m.account); await assert.rejects(m.payments.checkout(e.ctx, { plan: 'starter', operationId: randomUUID() }), rejected('PROVIDER_UNREACHABLE')); restore(); // the click that faulted: E superseded
  assert.equal((await m.f.store.get(`checkouts/stripe:${opE}`)).status, 'superseded');
  await assert.rejects(m.payments.checkout(e.ctx, { plan: 'starter', operationId: randomUUID() }), rejected('CHECKOUT_COMPLETING'), 'a new operation: the provider names E');
  assert.equal((await m.f.store.get(`checkouts/stripe:${opE}`)).status, 'pending'); assert.equal((await family(m.f, e.familyId)).checkoutIntent.stripe, opE); assert.equal(sessions(m.account), 1);
  sg = signed(m.f, event(m.f, 'checkout.session.completed', { object: 'checkout.session', payment_status: 'paid', customer: m.account.state.customer.id, subscription: 'sub_1', client_reference_id: opE, metadata: { familyId: e.familyId, checkoutId: opE } })); assert.equal((await m.payments.receive('stripe', sg.raw, sg.headers)).status, 'applied'); assert.equal((await family(m.f, e.familyId)).subscription.plan, 'starter');
  // the ending reads the subscription once more, by its id, right before the DELETE: a payment that landed since the list is seen
  const n = rig(), p = await pastDue(n, 'parentF'); const realFetchN = n.account.gw.fetch; let byId = 0;
  n.account.gw.fetch = async (url, init) => { if ((init?.method || 'GET') === 'GET' && /\/v1\/subscriptions\/sub_1$/.test(String(url))) { byId++; n.account.state.sub.status = 'active'; n.account.state.sub.current_period_end = Math.floor((n.f.now() + 30 * DAY) / 1000); } return realFetchN(url, init); };
  await assert.rejects(n.payments.checkout(p.ctx, { plan: 'family', ...op() }), rejected('PROVIDER_SUBSCRIPTION_PAID')); assert.equal(deletes(n.account), 0); assert.equal(byId, 1, 'one read by id'); n.account.gw.fetch = realFetchN;
});
test('refunds before completions, from every side: a first checkout and a trial family wait too; a refund pended under a customer the family no longer carries is processed again; a refund that waited on an abandoned checkout is the operator\'s after the session\'s day, and resolve-event closes a waiting row; a record ended long ago and paid again never skips the provider\'s truth', async () => {
  // a first checkout: the record names no subscription, the refund of the new one still waits
  const r = rig(), { f, account, payments } = r, a = await f.family('parentA', 0), opA = randomUUID(), A = await payments.checkout(a.ctx, { plan: 'starter', operationId: opA }); assert.ok(A.url);
  account.activate(PRICES.starter, undefined, { id: 'sub_A', metadata: { checkoutId: opA } });
  account.state.invoiceOf = { in_A: { id: 'in_A', object: 'invoice', subscription: 'sub_A' } }; account.state.charge = { id: 'ch_A', object: 'charge', customer: account.state.customer.id, amount: 500, amount_refunded: 500, refunded: true, invoice: 'in_A' };
  let sg = signed(f, event(f, 'refund.created', { object: 'refund', id: 're_A', charge: 'ch_A', amount: 500, status: 'succeeded' })); assert.deepEqual(await payments.receive('stripe', sg.raw, sg.headers), { status: 'requires_action', reason: 'CHECKOUT_PENDING' });
  f.advance(1000); sg = signed(f, event(f, 'checkout.session.completed', { object: 'checkout.session', payment_status: 'paid', customer: account.state.customer.id, subscription: 'sub_A', client_reference_id: opA, metadata: { familyId: a.familyId, checkoutId: opA } })); assert.equal((await payments.receive('stripe', sg.raw, sg.headers)).status, 'applied');
  const famA = await family(f, a.familyId); assert.deepEqual((famA.subscription.refunds || []).map((x) => x.amountCents), [500]); assert.notEqual(deriveState(famA.subscription, f.now()), 'active', 'no access on a refunded charge');
  // a trial family: the refund is never applied to the trial
  const g = rig(), b = await g.f.family('parentB', 0); await g.f.billing.startTrial(b.ctx, op()); const opB = randomUUID(), B = await g.payments.checkout(b.ctx, { plan: 'starter', operationId: opB }); assert.ok(B.url);
  g.account.activate(PRICES.starter, undefined, { id: 'sub_B', metadata: { checkoutId: opB } });
  g.account.state.invoiceOf = { in_B: { id: 'in_B', object: 'invoice', subscription: 'sub_B' } }; g.account.state.charge = { id: 'ch_B', object: 'charge', customer: g.account.state.customer.id, amount: 500, amount_refunded: 500, refunded: true, invoice: 'in_B' };
  sg = signed(g.f, event(g.f, 'refund.created', { object: 'refund', id: 're_B', charge: 'ch_B', amount: 500, status: 'succeeded' })); assert.deepEqual(await g.payments.receive('stripe', sg.raw, sg.headers), { status: 'requires_action', reason: 'CHECKOUT_PENDING' });
  assert.equal(deriveState((await family(g.f, b.familyId)).subscription, g.f.now()), 'trial', 'the trial is untouched');
  g.f.advance(1000); sg = signed(g.f, event(g.f, 'checkout.session.completed', { object: 'checkout.session', payment_status: 'paid', customer: g.account.state.customer.id, subscription: 'sub_B', client_reference_id: opB, metadata: { familyId: b.familyId, checkoutId: opB } })); assert.equal((await g.payments.receive('stripe', sg.raw, sg.headers)).status, 'applied');
  const famB = await family(g.f, b.familyId); assert.deepEqual((famB.subscription.refunds || []).map((x) => x.amountCents), [500]); assert.notEqual(deriveState(famB.subscription, g.f.now()), 'active');
  // a refund pended under a customer the family no longer carries (deleted at the provider, replaced by the next checkout): processed again after the completion, ignored as the old subscription's
  const k = rig(), d = await pastDue(k, 'parentD'), opD = randomUUID(); k.account.state.searchHidden = true; k.account.state.deletedCustomers = ['cus_live1']; k.account.state.sub.customer = 'cus_live1'; k.account.state.sub.status = 'canceled';
  assert.ok((await k.payments.checkout(d.ctx, { plan: 'family', operationId: opD })).url); assert.equal((await family(k.f, d.familyId)).providerCustomer.stripe, 'cus_live1_2');
  k.account.state.extraSubs = [sub(PRICES.family, k.f.now() + 30 * DAY, { id: 'sub_D', customer: 'cus_live1_2', metadata: { checkoutId: opD } })];
  sg = signed(k.f, event(k.f, 'checkout.session.completed', { object: 'checkout.session', payment_status: 'paid', customer: 'cus_live1_2', subscription: 'sub_D', client_reference_id: opD, metadata: { familyId: d.familyId, checkoutId: opD } })); assert.equal((await k.payments.receive('stripe', sg.raw, sg.headers)).status, 'applied'); assert.equal((await family(k.f, d.familyId)).subscription.providerSubscriptionRef, 'sub_D');
  k.f.advance(38 * DAY); k.account.state.extraSubs[0].status = 'past_due'; const d2 = await k.f.login('parentD'), opE2 = randomUUID(); assert.ok((await k.payments.checkout(d2.ctx, { plan: 'big', operationId: opE2 })).url, 'a third checkout, pending');
  k.account.state.invoiceOf = { in_1: { id: 'in_1', object: 'invoice', subscription: 'sub_1' } }; k.account.state.charge = { id: 'ch_1', object: 'charge', customer: 'cus_live1', amount: 500, amount_refunded: 500, refunded: true, invoice: 'in_1' };
  const reOld = event(k.f, 'refund.created', { object: 'refund', id: 're_1', charge: 'ch_1', amount: 500, status: 'succeeded' }); sg = signed(k.f, reOld); assert.deepEqual(await k.payments.receive('stripe', sg.raw, sg.headers), { status: 'requires_action', reason: 'CHECKOUT_PENDING' }, 'pended under the old customer');
  k.account.state.extraSubs.push(sub(PRICES.big, k.f.now() + 30 * DAY, { id: 'sub_E', customer: 'cus_live1_2', metadata: { checkoutId: opE2 } }));
  k.f.advance(1000); sg = signed(k.f, event(k.f, 'checkout.session.completed', { object: 'checkout.session', payment_status: 'paid', customer: 'cus_live1_2', subscription: 'sub_E', client_reference_id: opE2, metadata: { familyId: d.familyId, checkoutId: opE2 } })); assert.equal((await k.payments.receive('stripe', sg.raw, sg.headers)).status, 'applied');
  assert.deepEqual((await k.f.store.get(`billingEvents/stripe:${reOld.id}`)).outcome, { status: 'ignored', reason: 'OTHER_SUBSCRIPTION' }, 'processed again under the old customer, and the old subscription\'s'); assert.deepEqual((await k.f.store.get('billingCustomers/stripe:cus_live1')).pending, []);
  // a refund that waited on a checkout the parent abandoned: after the session's day it is the operator's ignored row; resolve-event closes a waiting row and takes it off the customer's list
  const m = rig(), e = await pastDue(m, 'parentE'), opM = randomUUID(); assert.ok((await m.payments.checkout(e.ctx, { plan: 'family', operationId: opM })).url);
  m.account.state.invoiceOf = { in_Z: { id: 'in_Z', object: 'invoice', subscription: 'sub_Z' } }; m.account.state.charge = { id: 'ch_Z', object: 'charge', customer: m.account.state.customer.id, amount: 300, amount_refunded: 300, refunded: true, invoice: 'in_Z' };
  const reZ = event(m.f, 'refund.created', { object: 'refund', id: 're_Z', charge: 'ch_Z', amount: 300, status: 'succeeded' }); sg = signed(m.f, reZ); assert.deepEqual(await m.payments.receive('stripe', sg.raw, sg.headers), { status: 'requires_action', reason: 'CHECKOUT_PENDING' });
  m.f.advance(25 * 60 * 60 * 1000); const later = await m.support.reprocess(e.familyId, OPERATOR); assert.ok(later.some((x) => x.id === reZ.id && x.status === 'ignored' && x.reason === 'OTHER_SUBSCRIPTION'), 'the checkout could no longer complete: the old subscription\'s refund');
  const m2 = rig(), e2 = await pastDue(m2, 'parentE2'), opM2 = randomUUID(); assert.ok((await m2.payments.checkout(e2.ctx, { plan: 'family', operationId: opM2 })).url);
  m2.account.state.invoiceOf = { in_Z: { id: 'in_Z', object: 'invoice', subscription: 'sub_Z' } }; m2.account.state.charge = { id: 'ch_Z', object: 'charge', customer: m2.account.state.customer.id, amount: 300, amount_refunded: 300, refunded: true, invoice: 'in_Z' };
  const reZ2 = event(m2.f, 'refund.created', { object: 'refund', id: 're_Z2', charge: 'ch_Z', amount: 300, status: 'succeeded' }); sg = signed(m2.f, reZ2); assert.equal((await m2.payments.receive('stripe', sg.raw, sg.headers)).reason, 'CHECKOUT_PENDING');
  const resolved = await m2.support.resolveEvent('stripe', reZ2.id, { operator: OPERATOR, outcome: 'no_action_needed', note: 'the old subscription\'s goodwill refund' }); assert.equal(resolved.previousOutcome, 'requires_action');
  assert.deepEqual((await m2.f.store.get(`billingCustomers/stripe:${m2.account.state.customer.id}`)).pending, [], 'off the list'); assert.deepEqual((await m2.support.reprocess(e2.familyId, OPERATOR)).filter((x) => x.id === reZ2.id), [], 'never processed again');
  // a record ended by this server long ago, then paid again: the ending is history, not the record's state — a lagging renewal is refused, never ended
  const q = rig(), s2 = await q.f.family('parentQ', 0); await subscribed(q, s2, 'starter');
  q.account.state.invoiceOf = { in_1: { id: 'in_1', object: 'invoice', subscription: 'sub_1' } }; q.account.state.charge = { id: 'ch_1', object: 'charge', customer: q.account.state.customer.id, amount: 500, amount_refunded: 500, refunded: true, invoice: 'in_1' };
  sg = signed(q.f, event(q.f, 'refund.created', { object: 'refund', id: 're_1', charge: 'ch_1', amount: 500, status: 'succeeded' })); assert.equal((await q.payments.receive('stripe', sg.raw, sg.headers)).status, 'applied'); assert.ok((await family(q.f, s2.familyId)).subscription.endedAt);
  q.f.advance(DAY); const s3 = await q.f.login('parentQ'), opQ = randomUUID(); assert.ok((await q.payments.checkout(s3.ctx, { plan: 'family', operationId: opQ })).url); assert.equal(q.account.state.sub.status, 'canceled', 'the record ahead: the returning checkout ended what still billed');
  const oldQ = q.account.state.sub; q.account.activate(PRICES.family, undefined, { id: 'sub_2', metadata: { checkoutId: opQ } }); q.account.state.extraSubs = [oldQ];
  sg = signed(q.f, event(q.f, 'checkout.session.completed', { object: 'checkout.session', payment_status: 'paid', customer: q.account.state.customer.id, subscription: 'sub_2', client_reference_id: opQ, metadata: { familyId: s2.familyId, checkoutId: opQ } })); assert.equal((await q.payments.receive('stripe', sg.raw, sg.headers)).status, 'applied');
  assert.equal((await family(q.f, s2.familyId)).subscription.endedAt, null, 'a paid subscription carries no ending');
  q.f.advance(38 * DAY); q.account.state.sub.current_period_end = Math.floor((q.f.now() + 20 * DAY) / 1000); // the provider renewed sub_2; its invoice.paid is late, so the record derives past_due
  const s4 = await q.f.login('parentQ'); await assert.rejects(q.payments.checkout(s4.ctx, { plan: 'big', ...op() }), rejected('PROVIDER_SUBSCRIPTION_PAID')); assert.equal(q.account.state.sub.status, 'active'); assert.equal(deletes(q.account), 1, 'only the first ending');
  // the resume path too: a click stalled, the record ended by a full refund meanwhile, the provider still billing — the retried click ends it
  const v = rig(), w = await pastDue(v, 'parentV'), opV = randomUUID(), restoreV = down(v.account); await assert.rejects(v.payments.checkout(w.ctx, { plan: 'family', operationId: opV }), rejected('PROVIDER_UNREACHABLE')); restoreV();
  v.account.state.invoiceOf = { in_1: { id: 'in_1', object: 'invoice', subscription: 'sub_1' } }; v.account.state.charge = { id: 'ch_1', object: 'charge', customer: v.account.state.customer.id, amount: 500, amount_refunded: 500, refunded: true, invoice: 'in_1' };
  sg = signed(v.f, event(v.f, 'refund.created', { object: 'refund', id: 're_1', charge: 'ch_1', amount: 500, status: 'succeeded' })); assert.equal((await v.payments.receive('stripe', sg.raw, sg.headers)).status, 'applied'); v.account.state.sub.status = 'active';
  assert.ok((await v.payments.checkout(w.ctx, { plan: 'family', operationId: opV })).url); assert.equal(v.account.state.sub.status, 'canceled'); assert.equal((await v.f.store.get(`checkouts/stripe:${opV}`)).endPrevious.result.status, 'active');
  // the stale rule's other clause: a resumed intent whose record names another subscription is stale, without a provider call
  const x = rig(), y = await pastDue(x, 'parentX'), opX = randomUUID(), restoreX = down(x.account); await assert.rejects(x.payments.checkout(y.ctx, { plan: 'family', operationId: opX }), rejected('PROVIDER_UNREACHABLE')); restoreX();
  await x.f.store.transaction(async (tx) => { const fam = await tx.get(`families/${y.familyId}`); tx.set(`families/${y.familyId}`, { ...fam, subscription: { ...fam.subscription, providerSubscriptionRef: 'sub_other' } }); }); x.account.calls.length = 0;
  await assert.rejects(x.payments.checkout(y.ctx, { plan: 'family', operationId: opX }), rejected('SUBSCRIPTION_CHANGED')); assert.equal((await x.f.store.get(`checkouts/stripe:${opX}`)).staleReason, 'STATE_MOVED'); assert.equal(x.account.calls.length, 0); assert.equal((await family(x.f, y.familyId)).checkoutIntent.stripe, null);
});
test('the deletion, from every side: every superseded session is expired, in pages beyond a hundred; a session paid after it was superseded is a subscription made, ended at deletion; a downgrade\'s answer is recorded without a proration', async () => {
  // an earlier superseded session whose click-path expiry faulted, then the deletion: both sessions expired; the older checkout's completion is the operator's
  const r = rig(), { f, account, payments, support } = r, a = await f.family('parentA', 0), opA = randomUUID(), A = await payments.checkout(a.ctx, { plan: 'starter', operationId: opA }); assert.ok(A.url);
  const realCancel = account.gw.cancelCheckout.bind(account.gw); account.gw.cancelCheckout = async (ref) => { account.gw.cancelCheckout = realCancel; throw Error('ECONNRESET'); }; // B's expiry of A faults
  const opB = randomUUID(), B = await payments.checkout(a.ctx, { plan: 'starter', operationId: opB }); assert.ok(B.url); assert.equal(account.state.expired.length, 0);
  await support.requestDeletion(a.ctx, op()); await support.executeDeletion(a.familyId, { operator: OPERATOR, force: true });
  assert.equal((await f.store.get(`checkouts/stripe:${opA}`)).expiredByDeletion.expired, true); assert.equal((await f.store.get(`checkouts/stripe:${opB}`)).expiredByDeletion.expired, true); assert.equal(account.state.expired.length, 2);
  account.activate(PRICES.starter, undefined, { metadata: { checkoutId: opA } });
  const lateA = event(f, 'checkout.session.completed', { object: 'checkout.session', payment_status: 'paid', customer: account.state.customer.id, subscription: 'sub_1', client_reference_id: opA, metadata: { familyId: a.familyId, checkoutId: opA } });
  const sg = signed(f, lateA); assert.deepEqual(await payments.receive('stripe', sg.raw, sg.headers), { status: 'reconciliation_required', reason: 'FAMILY_DELETED' }, 'a plain-superseded checkout of a deleted family');
  // a session paid after it was superseded (its expiry had faulted): the provider shows it complete, the subscription it made is ended at deletion and stands as the provider cancellation
  const g = rig(), b = await g.f.family('parentB', 0), opC = randomUUID(), C = await g.payments.checkout(b.ctx, { plan: 'starter', operationId: opC }); assert.ok(C.url);
  const realCancelG = g.account.gw.cancelCheckout.bind(g.account.gw); g.account.gw.cancelCheckout = async (ref) => { g.account.gw.cancelCheckout = realCancelG; throw Error('ECONNRESET'); };
  assert.ok((await g.payments.checkout(b.ctx, { plan: 'starter', operationId: randomUUID() })).url);
  g.account.activate(PRICES.starter, undefined, { id: 'sub_C', metadata: { checkoutId: opC } }); g.account.state.sessionOf = { [C.providerCheckoutRef]: { status: 'complete', subscription: 'sub_C' } }; // paid on the superseded page
  let sg2 = signed(g.f, event(g.f, 'checkout.session.completed', { object: 'checkout.session', payment_status: 'paid', customer: g.account.state.customer.id, subscription: 'sub_C', client_reference_id: opC, metadata: { familyId: b.familyId, checkoutId: opC } })); assert.deepEqual(await g.payments.receive('stripe', sg2.raw, sg2.headers), { status: 'rejected', reason: 'CHECKOUT_SUPERSEDED' });
  await g.support.requestDeletion(b.ctx, op()); const recB = await g.support.executeDeletion(b.familyId, { operator: OPERATOR, force: true });
  const paid = await g.f.store.get(`checkouts/stripe:${opC}`); assert.equal(paid.expiredByDeletion.reason, 'SESSION_COMPLETED'); assert.equal(paid.expiredByDeletion.subscriptionRef, 'sub_C'); assert.equal(paid.endedByDeletion.cancelled, true); assert.equal(g.account.state.sub.status, 'canceled');
  assert.equal(recB.providerCancellation.status, 'cancelled'); assert.equal(recB.providerCancellation.checkoutId, opC); assert.ok(!(await g.support.inspectAll({ operator: OPERATOR })).findings.some((x) => x.family === b.familyId));
  // more than a hundred checkouts: every one of them expired, none left to the first page
  const h = rig(), c = await h.f.family('parentC', 0); for (let i = 0; i < 105; i++) assert.ok((await h.payments.checkout(c.ctx, { plan: 'starter', operationId: `${String(i).padStart(8, '0')}-0000-4000-8000-000000000000` })).url);
  await h.support.requestDeletion(c.ctx, op()); await h.support.executeDeletion(c.familyId, { operator: OPERATOR, force: true });
  const all = (await h.support.pagedBy(h.f.store, 'checkouts', 'familyId', c.familyId)).rows; assert.equal(all.length, 105); assert.ok(all.every((x) => x.expiredByDeletion?.expired === true), 'every session expired by the deletion');
  // a downgrade's answer (schedulePlan) is recorded with its reference and no proration when the finalisation dies
  const k = rig(), d = await k.f.family('parentD', 0); await subscribed(k, d, 'family'); const opD = randomUUID(), realParent = k.payments.billing.parent.bind(k.payments.billing), realSchedule = k.account.gw.schedulePlan.bind(k.account.gw); let armed = false;
  k.account.gw.schedulePlan = async (args) => { const answer = await realSchedule(args); armed = true; return answer; };
  k.payments.billing.parent = async (...args) => { if (armed) { armed = false; throw Error('ECONNRESET'); } return realParent(...args); };
  await assert.rejects(k.payments.changePlan(d.ctx, { plan: 'starter', operationId: opD }), /ECONNRESET/); k.payments.billing.parent = realParent; k.account.gw.schedulePlan = realSchedule;
  const di = await k.f.store.get(`billingChangeIntents/stripe:${opD}`); assert.equal(di.status, 'creating'); assert.equal(di.kind, 'downgrade'); assert.ok(di.providerAnsweredAt); assert.equal(di.providerOperationRef, 'sub_1'); assert.equal(di.proration, null);
  assert.equal((await k.payments.changePlan(d.ctx, { plan: 'starter', operationId: opD })).kind, 'downgrade'); assert.equal((await k.f.store.get(`billingChangeIntents/stripe:${opD}`)).status, 'applied');
});
