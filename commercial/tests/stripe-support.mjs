...
export function stripeAccount(f, { customerId = 'cus_live1' } = {}) {
  ...
  const routes = {
    ...
    'POST /v1/checkout/sessions/': (body, calls) => { const path = calls.at(-1).path, id = path.split('/').at(-2); if ((state.strictExpire && state.expired.includes(path)) || (state.sessionOf || {})[id]?.status === 'complete') return { status: 400, json: { error: { code: 'invalid_request_error', type: 'invalid_request_error' } } }; state.expired.push(path); return { status: 'expired' }; }, // strictExpire: Stripe refuses to expire a session no longer open; sessionOf: a session's status (complete: paid) and subscription
    'GET /v1/checkout/sessions/': (body, calls) => { const id = calls.at(-1).path.split('/').pop(), s = (state.sessionOf || {})[id]; return { id, object: 'checkout.session', status: s?.status || (state.expired.some((p) => p.includes(`/${id}/`)) ? 'expired' : 'open'), subscription: s?.subscription || null }; },
