// Olympia's eight moons (server/questions/olympia): every heat has the moon's real shape, every question is well formed
...
test('the eight moons, all open, in the owner\'s order (SEAMO, AMO, SMC and WMI first, then HK, BKK and PHI, then DC), ids unique, and the public shape carries no generator', () => {
...
test('every heat is ten questions in the moon\'s real shape: SEAMO four 3-mark and four 4-mark MC then two 6-mark SA, five options with "None of the above" on every paper; AMO six 3-mark MC of five then two 5-mark and two 6-mark SA; SMC two 2-mark MC of four then eight SA climbing to 4 marks; WMI three 6-mark and two 8-mark logic then five 10-mark applications, all MC of four; AMC 8 ten MC of five options in three climbing bands', () => {
  for (const { m, y, qs } of heats(6)) {
    ...
    if (m.id === 'us') { assert.deepEqual(kinds, [...Array(6).fill('mc'), ...Array(4).fill('sa')]); assert.deepEqual(sections, [...Array(6).fill('M3'), 'M5', 'M5', 'M6', 'M6'], `${m.id} ${y}: the paper's tiers`); for (const q of qs.slice(0, 6)) { assert.equal(q.display.choices.length, 5); assert.ok(!q.display.choices.includes(NONE)); } }
    if (m.id === 'sg') { assert.deepEqual(kinds, ['mc', 'mc', ...Array(8).fill('sa')]); assert.deepEqual(sections, ['M2', 'M2', 'M2', 'M2', 'M2', 'M3', 'M3', 'M3', 'M4', 'M4'], `${m.id} ${y}: the paper's tiers`); for (const q of qs.slice(0, 2)) assert.equal(q.display.choices.length, 4); for (const q of qs.slice(2)) assert.notEqual(q.answer.type, 'frac', `the real paper types no fractions: ${q.display.text}`); }
