...
export async function uiFixture(t, { family = true, signedIn = true } = {}) {
  ...
  const server = createApp(f.service, cfg, { learning: f.learning }); server.listen(0, '127.0.0.1'); await once(server, 'listening');
  ...
  const context = vm.createContext({ document, window: { BroadcastChannel: Channel }, BroadcastChannel: Channel, crypto: webcrypto, fetch: fetchForPage, console,
