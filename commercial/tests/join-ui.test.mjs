// /join, the page a social post points at (the owner's request of 12 Sep 2026). Before the doors open on 14 September it takes
// an address and the permission to write to it, and nothing else: no password, no mobile code, no child. The SMS code lives
// where it always did, inside sign-up. From 14 September the same address leads into that sign-up instead, and the page states
...
const WIB_BEFORE = Date.parse('2026-09-13T04:00:00Z'); // 11:00 in Jakarta, the day before the doors open
...
test('before the doors open, /join explains the grid and asks for an address alone — no password, no mobile code, no child', async (t) => {
  ...
  assert.ok(text(h).includes('14 September 2026'), 'it names the day it opens');
...
test('from 14 September the same page starts the trial instead, and names the one moment it ends for everyone', async (t) => {
