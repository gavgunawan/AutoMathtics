...
// and the box comes back ticked only for the account that ticked it on this device. The check of a parent action and
// the verify-mobile step never ask.
...
// the pre-tick waits on a hash of the email: give it time to land before looking
const ticked = async (h) => { for (let i = 0; i < 100 && !box(h)?.checked; i++) await pause(5); };
function challenge(h, uid, email) {
  h.setAuth(uid, { signIn: async () => ({ stage: 'challenge', phone: '+*******7890', email }), sendCode: async () => {}, nextSendAt: async () => 0,
    confirmCode: async () => ({ stage: 'ready', idToken: h.f.token(uid) }) });
}
async function signingIn(t, options = {}) {
  const h = await uiFixture(t, { signedIn: false, ...options });
  await h.f.family('parentA', 2);
  challenge(h, 'parentA', 'synthetic@example.test');
  return h;
}

test('ticked on the sign-in SMS step: the device stays signed in for 30 days, Mission Control says until when, and the box comes back ticked for that account', async (t) => {
  ...
  await pause(50); assert.ok(!box(h).checked, 'unticked the first time');
  box(h).checked = true;
  ...
  const tag = store.map.get('automathtics.remember');
  assert.match(tag, /^[0-9a-f]{64}$/, 'a hash of the account, not the email'); assert.ok(!tag.includes('synthetic'));

  await h.click('Sign out'); await h.idle();
  h.f.advance(1000); // a sign-in after a sign-out needs a token newer than the sign-out
  await h.submitLogin(); await ticked(h);
  assert.equal(box(h).checked, true, 'the box comes back ticked for the same account');
  box(h).checked = false;
  await h.click('Verify code'); await h.idle();
...
test('another account on the same device finds the box unticked (review of PR #44)', async (t) => {
  const store = storage(), h = await signingIn(t, { storage: store });
  await h.submitLogin(); await settle(); box(h).checked = true;
  await h.click('Verify code'); await h.idle();
  assert.ok(store.map.get('automathtics.remember'));
  await h.click('Sign out'); await h.idle();
  h.f.advance(1000);
  challenge(h, 'parentB', 'another.parent@example.test');
  await h.submitLogin(); await settle(); await pause(100);
  assert.ok(h.root.textContent.includes('Remember this device for 30 days'));
  assert.ok(!box(h).checked, 'not pre-ticked for someone else');
});
...
test('a browser that blocks site data still signs in: the box starts unticked and a tick still counts', async (t) => {
  ...
  await h.submitLogin(); await settle(); await pause(50);
