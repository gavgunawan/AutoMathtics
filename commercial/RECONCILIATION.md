...
| `PROVIDER_ATTENTION` (sweep) | a parent's action was refused on the provider's truth within the last seven days: `MULTIPLE_PROVIDER_SUBSCRIPTIONS`, `PROVIDER_SUBSCRIPTION_LIVE` (a live subscription naming no checkout of ours — the dashboard's — or one whose checkout never reached the provider), `CHECKOUT_COMPLETING` (a checkout of ours completing — the provider's subscription names it, or its bank debit is still clearing — reinstated as the family's live checkout; its completion applies when it lands, and one the inbox rejected meanwhile is processed again by the reinstatement), `PROVIDER_SUBSCRIPTION_NOT_FOUND` (the provider could not find the subscription to end), `PROVIDER_SUBSCRIPTION_PAID` (paid and current at the provider while the family's record says otherwise: its `invoice.paid` is still on its way, or was lost — `reprocess` after `stripe events resend`); the family carries the mark and an audit row `billing.refused` | `reconcile-provider FAMILY`: it names what the provider holds; resolve it there (cancel the wrong subscription, or wait for the pending checkout's `checkout.session.completed`), reconcile again — a clean result clears the mark. A `CHECKOUT_SUPERSEDED` rejection in the inbox for a checkout that is `pending` again: `stripe events resend` it |
| `CHECKOUT_PENDING` (inbox, `requires_action`) | a refund or a dispute of a subscription the family's record does not name yet, delivered while a checkout of the family is still pending: it may be the new subscription's, refunded before its completion landed | nothing: it is processed again right after the completion (the family's own refund applies; an old subscription's is ignored as `OTHER_SUBSCRIPTION`). Still waiting after the checkout lapsed: `INBOX_WAITING_STALE` names it; `resolve-event` it |
