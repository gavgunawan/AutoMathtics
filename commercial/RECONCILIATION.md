...
| `PAUSE_NOT_ECHOED`, `PAUSE_OVERDUE` | a pause the provider has never echoed after a day, or one whose resume date passed a week ago with no invoice since | `reconcile-provider`: a pause the provider does not have will charge the family it was told would not be charged, and one that should have ended is waiting for money that is not coming. The sweep also counts paused families (`counts.paused`), which are never a finding on their own and never churn |
...
| `PAUSE_MISMATCH`, `PAUSE_RESUME_MISMATCH`, `PAUSE_BEHAVIOUR` | collection is paused at one end and not the other, the resume dates differ, or the provider's pause is not `void` (so an invoice raised now would be collected later) | the parent's own flow (`/api/billing/pause`, `/api/billing/resume`) sets both; if the dashboard alone was changed, set it back there, or let the provider's `customer.subscription.updated` echo land (`reprocess FAMILY_UUID`) — the provider is the authority on a pause. A pause that is not `void` must be set to `void`, or ended and made again through the app |
...
| `PROVIDER_ATTENTION` (sweep) | a parent's action was refused on the provider's truth within the last seven days: `MULTIPLE_PROVIDER_SUBSCRIPTIONS`, `PROVIDER_SUBSCRIPTION_LIVE` (a live subscription naming no checkout of ours — the dashboard's — or one whose checkout never reached the provider), `CHECKOUT_COMPLETING` (a checkout of ours completing — the provider's subscription names it, whichever click superseded it, or its bank debit is still clearing — reinstated as the family's live checkout; its completion applies when it lands, one the inbox rejected meanwhile is processed again by the reinstatement, and a failed debit releases the checkout), `PROVIDER_SUBSCRIPTION_NOT_FOUND` (the provider could not find the subscription to end), `PROVIDER_SUBSCRIPTION_PAID` (paid and current at the provider while the family's record says otherwise: its `invoice.paid` is still on its way, or was lost — `reprocess` after `stripe events resend`); the family carries the mark and an audit row `billing.refused` | `reconcile-provider FAMILY`: it names what the provider holds; resolve it there (cancel the wrong subscription, or wait for the pending checkout's `checkout.session.completed`), reconcile again — a clean result clears the mark. A `CHECKOUT_SUPERSEDED` rejection in the inbox for a checkout that is `pending` again: `stripe events resend` it |
| `CHECKOUT_PENDING` (inbox, `requires_action`) | a refund or a dispute of a subscription the family's record does not name (a record naming none included), delivered while the family's live checkout can still complete — within the day a hosted session lives: it may be the new subscription's, refunded before its completion landed | nothing: it is processed again right after the completion, under every reference the family ever carried (the family's own refund applies; an old subscription's is ignored as `OTHER_SUBSCRIPTION`), and past that day it is ignored as the old subscription's by the next `reprocess`. Still waiting: `INBOX_WAITING_STALE` names it; `resolve-event` closes it and takes it off the customer's list |
