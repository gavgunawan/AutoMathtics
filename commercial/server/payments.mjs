...
import { Fault, fail, equal, object, text, sha256 } from './security.mjs';
...
export class Payments {
  ...
  async receive(providerName, rawBody, headers) {
    ...
    return this.store.transaction(async (tx) => {
      ...
      if (seen) { if (seen.fingerprint !== fingerprint) fail(409, 'IDEMPOTENCY_CONFLICT'); return { ...seen.outcome, replayed: true }; }
      ...
      tx.set(inboxPath, { provider: gw.name, providerEventId: ev.id, type: ev.type, at: ev.at, receivedAt: now, customer: ev.customer, familyId, data: ev.data, fingerprint, outcome });
